...
packaging / ライブラリ名 / 概要 / 提供元 / ライセンス / 種別(OSS/ベンダー) /
プロジェクト URL / 座標の判定元 / 推定 / 取得 URL / 取得元リポジトリ / URL 確認 /
サイズ / SHA-1 / 配置ディレクトリ / 備考`。「凡例」シートに色分けとカラム説明を同梱。

**種別(OSS/ベンダー)**: 各 JAR がオープンソースか商用ベンダー製かを推定して色付きで表示。
判定は **ライセンス → groupId プレフィックス → ベンダー名** の順（`OSS`=緑 / `ベンダー系`=橙 /
`要確認`=灰）。ライセンスが最優先の確定材料であり、確定には条項の確認を推奨。
